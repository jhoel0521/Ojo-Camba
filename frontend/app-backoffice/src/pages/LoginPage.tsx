    ...
    <div className="min-h-screen flex items-center justify-center bg-lienzo font-pirai p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-catedral rounded-hero mb-4">
            <MapPin className="w-8 h-8 text-lienzo" />
          </div>
          <h1 className="text-2xl font-semibold text-tierra">Ojo Camba</h1>
          <p className="text-sm text-arena mt-1">BackOffice — Acceso moderadores</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-perla rounded-3xl-3 p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-ladrillo uppercase tracking-wide mb-1.5">
              Email
            </label>
            <div className="relative">
              ...
              <input
                {...register('email')}
                type="email"
                placeholder="moderador@ojocamba.bo"
                autoComplete="email"
                className="bg-lienzo border border-arcilla rounded-3xl-3 pl-10 pr-5 py-3.5 text-sm text-tierra placeholder:text-almendra w-full focus:outline-none focus:border-caoba transition-colors"
              />
            </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 bg-catedral text-perla font-medium text-sm px-8 py-3.5 rounded-3xl-3 shadow-md hover:bg-tierra disabled:opacity-60 transition-all"
          >
            {loading ? (
