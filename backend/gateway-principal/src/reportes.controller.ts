...
export class ReportesController {
  ...
  @Post()
  create(@Body() dto: {
    device_id: string; lat: number; lng: number; categoria_id: number;
    gravedad?: string; imagen_base64?: string;
  }) {
    return firstValueFrom(this.client.send(TCP_PATTERNS.REGISTER.CREATE_REPORT, dto));
  }

  @Get()
  list(@Query() query: { page?: string; limit?: string; estado?: string; categoria_id?: string; h3_res_8?: string }) {
    return firstValueFrom(this.client.send(TCP_PATTERNS.REGISTER.LIST_REPORTS, {
  ...
  heatmap(@Query('resolution') resolution?: string) {
    return firstValueFrom(this.client.send(TCP_PATTERNS.REGISTER.GET_HEATMAP, {
  ...
  get(@Param('id') id: string) {
    return firstValueFrom(this.client.send(TCP_PATTERNS.REGISTER.GET_REPORT, { report_id: parseInt(id, 10) }));
  }
}
